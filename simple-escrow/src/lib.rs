...
impl Escrow{
    ...
    fn confirm_delivery(&mut self){
        assert_eq!(self.state, EscrowState::AwaitingDelivery, "State must be AwaitingDelivery");
        assert_eq!(msg::source(), self.buyer, "The message sender must be a buyer");

        msg::send(self.seller, String::default(), self.price).expect("Error in transfering money to seller");
